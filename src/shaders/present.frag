...
vec2 glitchUv(vec2 uv, float t) {
  ...
  if (b > 0.968) {
    uv += (vec2(hash21(block + 1.3), hash21(block + 4.1)) - 0.5) * 0.08;
  }
  if (mod(floor(uv.y * uResolution.y), 3.0) < 0.5 && hash21(vec2(floor(t * 5.0), 8.0)) > 0.55) {
    uv.x += 0.0035;
  }
  return uv;
}
...
void main() {
  ...
  float split = 0.0035 + 0.004 * step(0.92, hash21(vec2(floor(t * 6.0), 2.2)));
  vec3 col;
  ...
  vec2 gUvB = glitchUv(uv + vec2(0.03, -0.04), t + 2.4);
  vec3 colB;
  ...
  float ringC = frameRing(uv, vec2(0.38, 0.58), vec2(0.16, 0.14), 0.011);

  vec2 p = (uv - 0.5) * vec2(aspect, 1.0);
  float poles = 0.0;
  poles += pole(p, vec2(-0.95, -0.72), vec2(0.82, 0.88), 0.007);
  poles += pole(p, vec2(-0.6, 0.9), vec2(0.92, -0.55), 0.0055);
  poles += pole(p, vec2(0.05, -0.92), vec2(-0.2, 0.95), 0.0045);
